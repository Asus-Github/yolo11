...
## 8. +W 完整训练结果（2026-06-04）

**配置**：`yolo11n.yaml` + WIoU；epochs=300（patience=50 早停）；imgsz=640；optimizer=SGD lr0=0.01；amp=True；nbs=64；**实际 batch=62**（AutoBatch 0.85 探测）。
...
| mAP@0.5 | 86.0% |
| mAP@0.5:0.95 | 60.2% |
| FPS（full pipeline）| 200 |
| GFLOPs | 6.4（架构未变）|
| Params | 2.6M |

**对比 baseline-v2**（CIoU，相同 epochs/imgsz/optimizer，batch=64）：mAP50 +0.3%，mAP50-95 +0.1%。提升幅度小，在随机性区间内，但趋势正向。

**复现命令**：
```bash
ssh autodl
tmux new -s w_train  # 注意：默认会在命令结束后销毁 session
source /root/miniconda3/etc/profile.d/conda.sh && conda activate base
cd /root/autodl-tmp/ultralytics
python train_variant.py -c yolo11n.yaml --iou wiou -n +W
```

> **关于 batch 一致性**：baseline-v2=64，+W=62（AutoBatch），后续变体随结构增加显存会更小。NBS 机制（accumulate=round(nbs/batch)）只在 batch ≪ nbs 时触发；62 vs 64 不触发累积，仅 weight_decay × 0.97。理论影响 < 0.5% mAP，主结论不受影响。**所有训练后须记录实际 batch（args.yaml 里有），论文中作为一列汇报**。

---

## 9. +T 集成与训练（Triplet Attention，2026-06-04）

### 9.1 改动文件

| # | 文件 | 操作 | 行为 |
|---|---|---|---|
| 1 | `ultralytics/nn/modules/conv.py` | 末尾追加 3 个类 | `ZPool`, `AttentionGate`, `TripletAttention` |
| 2 | `ultralytics/nn/modules/__init__.py` | 加导入和 `__all__` | 暴露 `TripletAttention` |
| 3 | `ultralytics/nn/tasks.py` | 顶部加 import + `parse_model` 加分支 | `c1=ch[f]; c2=c1; args=[c1]` |
| 4 | `ultralytics/cfg/models/11/yolo11-t.yaml` | **新建** | 在 backbone P5 末端、SPPF 前插入 Triplet |

### 9.2 关键设计点

| 点 | 为什么 |
|---|---|
| 插在 backbone P5 末端（layer 9，SPPF 前）| 通道最高（1024×scale），注意力收益最大；位于 backbone 末段不破坏 PAN-FPN |
| `c2 = c1`（通道不变）| Triplet 只做 attention 加权，三支平均输出与输入同形状 |
| 三支用 `permute` 实现跨维交互 | (C,H) 维和 (C,W) 维通过 permute 复用同一 AttentionGate |
| `kernel_size=7` BN 后 sigmoid | 论文默认；7×7 大感受野捕捉跨维模式 |
| **YAML 层索引整体 +1** | 关键易错点：原 SPPF=9, C2PSA=10, Detect 引用 [16,19,22]；插 Triplet 后 SPPF=10, C2PSA=11, Detect=[17,20,23]，`[-1,10]→[-1,11]`、`[-1,13]→[-1,14]`。**任何在中间插层都要重新数索引** |

### 9.3 Smoke test（autodl）

```python
from ultralytics.nn.tasks import DetectionModel
m = DetectionModel('ultralytics/cfg/models/11/yolo11-t.yaml', ch=3, nc=8)
# Layers: 25, Params: 2,591,700 (~2.59M)  ← 比 baseline 2.58M 多 ~10K（极小）
import torch; m.eval()
y = m(torch.randn(2,3,640,640))
# y[0].shape == (2, 12, 8400)  forward OK
```
...
> ⚠️ **tmux 教训**（来自 +W 训练）：默认 `tmux new -d -s xx 'cmd'` 在 cmd 结束后会销毁 session。
> 想保留窗口看 log，**两种做法选一**：
> 1. 启动后 `tmux set-option -t xx remain-on-exit on`（本次采用）
> 2. 启动命令最后加 `; bash` 留 shell

### 9.5 训练结果

（待训练完成后填写：实际 batch、训练时间、最终 P/R/mAP50/mAP50-95、FPS、与 baseline 对比）
